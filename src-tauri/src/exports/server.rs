...
pub async fn insert_server(server: Server) -> Result<HttpResponse, String> {
    Task::task_param_future::<Server, _, _>(server, |server| async move { Server::insert(&*server).await }).await
}

/// 更新服务器
#[tauri::command]
pub async fn update_server(server: Server) -> Result<HttpResponse, String> {
    Task::task_param_future::<Server, _, _>(server, |server| async move { Server::update(&*server).await }).await
}
...
pub async fn delete_server(id: String) -> Result<HttpResponse, String> {
    ...
    Task::task_param_future::<Server, _, _>(server, |server| async move { Server::delete(&*server).await }).await
}
...
pub async fn get_server_detail(id: String) -> Result<HttpResponse, String> {
    let mut server = Server::default();
    server.id = id.to_string();
    Task::task_param_future::<Server, _, _>(server, |server| async move { Server::get_by_id(&*server).await }).await
}
